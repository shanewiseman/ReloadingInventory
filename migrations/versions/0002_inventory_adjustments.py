...
def upgrade():
    ...
    lot_columns = {column["name"] for column in inspector.get_columns("inventory_lot")}
    if "adjustment_quantity" not in lot_columns:
    ...
    if "inventory_adjustment" not in inspector.get_table_names():
        op.create_table(
